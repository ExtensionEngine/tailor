...
Axios.prototype.submitForm = function (url, fields, options) {
  var action = buildFullPath(this.defaults.baseURL, url);
...
var client = axios.create(config); // Attach additional instance without interceptors
...
var isAuthError = function isAuthError(err) {
  ...
  return [FORBIDDEN, UNAUTHORIZED].includes((_err$response = err.response) === null || _err$response === void 0 ? void 0 : _err$response.status);
};
...
export { activity, asset, auth, client, contentElement, extractData, feed, repository, revision, tag, user };
